...
class VLLMEngine(RayActor):
    ...
    def update_weights_from_tensor(
        ...
        """Load colocated weights via CUDA IPC handles.

        Deserializes SGLang-style flattened tensor buckets, moves them to GPU,
        builds CUDA IPC handles, and posts them to vLLM's ``/update_weights``
        endpoint.  Requires the vLLM process to have been started with
        ``--weight-transfer-config`` backend ``ipc`` (set automatically when
        ``--colocate`` is active).
        """
        import pickle
